...
export const Mission: React.FC<MissionProps> = ({ mission }) => {
	const { data: destinations, isFetching: destinationsIsFetching } =
		useDestinations()
	...
	const destinationLabel =
		destinations?.find(
			(destination) => destination.id === mission.destination,
		)?.label ?? null
	...
	return (
		<Link
			...
			data-testid={`mission mission-${mission.id}`}
		>
			...
			<div className={styles.missionBody}>
				...
				<div className={styles.members} data-testid="members">
					{mission.members.map((member) => (
						<span key={member.type}>
							...
							&nbsp;
							<span data-testid={`member-${member.type}-count`}>
								{member.count}
							</span>
						</span>
					))}
				...
				<div className="smaller-text" data-testid="departure">
					...
					<span
						className={
							Math.abs(diffDate) === 0 ? styles.departed : ''
						}
					>
