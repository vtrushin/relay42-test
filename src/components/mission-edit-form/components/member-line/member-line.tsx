...
export const MemberLine: React.FC<MemberProps> = ({
	...
	const canDelete =
		type === 'engineer' ||
		(type === 'passenger' && selectedPassengersNumber > 1)
				...
				<Fragment key="pilot">
					<FormField label="Experience">
						...
						<ErrorMessage
							...
							render={({ message }) => (
								<div
									className="error-text smaller-text"
									data-testid="error"
								>
									{message}
								</div>
		...
		<>
			<FormField label="Type">
				<Select
					...
					disabled={!canDelete}
				>
					<option value="pilot" disabled={!canSelectPilot}>
						👨‍✈️ Pilot
					</option>
					<option value="engineer" disabled={!canSelectEngineer}>
						👷‍♂️ Engineer
					</option>
